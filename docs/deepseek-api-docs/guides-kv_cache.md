...
-   [English](/guides/kv_cache){.dropdown__link target="_self"
    rel="noopener noreferrer" lang="en"}
-   [中文（中国）](/zh-cn/guides/kv_cache){.dropdown__link
...
-   ::: menu__list-item-collapsible
    ...
    -   [Tool Calls](/zh-cn/guides/tool_calls){.menu__link tabindex="0"}
...
-   [上下文硬盘缓存]{.breadcrumbs__link itemprop="name"}
...
DeepSeek API
[上下文硬盘缓存技术](/zh-cn/news/news0802)对所有用户默认开启，用户无需修改代码即可享用。
...
### 例一：长文本问答[​](#例一长文本问答 "例一：长文本问答的直接链接"){.hash-link aria-label="例一：长文本问答的直接链接"} {#例一长文本问答 .anchor .anchorWithStickyNavbar_YAqC}
...
在上例中，两次请求都有相同的**前缀**，即 `system` 消息 + `user` 消息中的
`<财报内容>`。在第二次请求时，这部分前缀会计入"缓存命中"。

------------------------------------------------------------------------

### 例二：多轮对话[​](#例二多轮对话 "例二：多轮对话的直接链接"){.hash-link aria-label="例二：多轮对话的直接链接"} {#例二多轮对话 .anchor .anchorWithStickyNavbar_YAqC}
...
在上例中，第二次请求可以复用第一次请求**开头**的 `system` 消息和 `user`
消息，这部分会计入"缓存命中"。

------------------------------------------------------------------------

### 例三：使用 Few-shot 学习[​](#例三使用-few-shot-学习 "例三：使用 Few-shot 学习的直接链接"){.hash-link aria-label="例三：使用 Few-shot 学习的直接链接"} {#例三使用-few-shot-学习 .anchor .anchorWithStickyNavbar_YAqC}

在实际应用中，用户可以通过 Few-shot
学习的方式，来提升模型的输出效果。所谓 Few-shot
学习，是指在请求中提供一些示例，让模型学习到特定的模式。由于 Few-shot
一般提供相同的上下文前缀，在硬盘缓存的加持下，Few-shot 的费用显著降低。
...
在上例中，使用  了
4-shots。两次请求只有最后一个问题不一样，第二次请求可以复用第一次请求中前
4 轮对话的内容，这部分会计入"缓存命中"。

------------------------------------------------------------------------

## 查看缓存命中情况[​](#查看缓存命中情况 "查看缓存命中情况的直接链接"){.hash-link aria-label="查看缓存命中情况的直接链接"} {#查看缓存命中情况 .anchor .anchorWithStickyNavbar_YAqC}

在 DeepSeek API 的返回中，我们在 `usage`
字段中增加了两个字段，来反映请求的缓存命中情况：

1.  `prompt_cache_hit_tokens`：本次请求的输入中，缓存命中的 tokens
    数（0.1 元 / 百万 tokens）

2.  `prompt_cache_miss_tokens`：本次请求的输入中，缓存未命中的 tokens
    数（1 元 / 百万 tokens）

## 硬盘缓存与输出随机性[​](#硬盘缓存与输出随机性 "硬盘缓存与输出随机性的直接链接"){.hash-link aria-label="硬盘缓存与输出随机性的直接链接"} {#硬盘缓存与输出随机性 .anchor .anchorWithStickyNavbar_YAqC}

硬盘缓存只匹配到用户输入的前缀部分，输出仍然是通过计算推理得到的，仍然受到
temperature
等参数的影响，从而引入随机性。其输出效果与不使用硬盘缓存相同。

## 其它说明[​](#其它说明 "其它说明的直接链接"){.hash-link aria-label="其它说明的直接链接"} {#其它说明 .anchor .anchorWithStickyNavbar_YAqC}
...
::: {.col .col--12}
[](/zh-cn/guides/tool_calls){.pagination-nav__link
...
[](/zh-cn/guides/anthropic_api){.pagination-nav__link
...
-   [例一：长文本问答](#例一长文本问答){.table-of-contents__link
    .toc-highlight}
-   [例二：多轮对话](#例二多轮对话){.table-of-contents__link
    .toc-highlight}
-   [例三：使用 Few-shot
    学习](#例三使用-few-shot-学习){.table-of-contents__link
    .toc-highlight}
-   [查看缓存命中情况](#查看缓存命中情况){.table-of-contents__link
    .toc-highlight}
-   [硬盘缓存与输出随机性](#硬盘缓存与输出随机性){.table-of-contents__link
    .toc-highlight}
-   [其它说明](#其它说明){.table-of-contents__link .toc-highlight}
:::
:::
:::
:::
